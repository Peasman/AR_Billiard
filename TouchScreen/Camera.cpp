...
void Camera::consoleOutput(std::string description){
	std::cout << description << " | Press any key to continue . . ." << std::endl;
	getchar();
}

void Camera::showChessboard(){
	//consoleOutput("Hier sollte das Schachbrett gezeigt werden!");
	cv::Mat chessboard;
	chessboard = cv::imread("C:\\Users\\fp19\\Documents\\Fachprojekt\\AR_Billiard\\Chess_Board.png");
	if (!chessboard.data){
		consoleOutput("Could not open or find the image!");
	}
	else{
		cv::namedWindow("Chessboard Show", CV_WINDOW_AUTOSIZE);
...
void Camera::mainLoop()
{
	...
	if (videoCapture.isOpened()){
		consoleOutput("ERROR: No camera detected!");
	}
	else
	{
		while (!_terminate)
