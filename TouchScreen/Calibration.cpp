...
cv::Point2f Calibration::mapPoint(cv::Point2f blackpoint)//Punkt in Kamera
{
	...
	{
		//std::cout << "CALIB: >> Punkt IST im Spielfeldbereich" << std::endl;
		//std::cout << "x=" << blackpoint.x << " y=" << blackpoint.y << std::endl;
		...
		std::cout << "CALIB: >> Punkt Game Scaled x=" << xBlack << " y=" << yBlack << std::endl;
...
{/*
	...
	//cv::warpAffine(undistorted, rotated, r1, distorted.size);
